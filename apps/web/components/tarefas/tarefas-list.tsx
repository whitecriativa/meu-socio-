...
import { Star, Zap, Clock, ArrowDown } from 'lucide-react'
import { Card, CardContent } from '@/components/ui/card'
import { toggleTask } from '@/app/(dashboard)/tarefas/actions'
...
const QUADRANT_META: Record<Quadrant, { label: string; icon: React.ReactNode; bg: string }> = {
  ...
  important_not_urgent: { label: 'Importante',           icon: <Star className="w-3 h-3" />,      bg: 'bg-[#5B3FD4]/10 text-[#5B3FD4]' },
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="flex flex-col items-end">
            <div className="flex items-center gap-1.5 bg-[#5B3FD4]/10 text-[#5B3FD4] px-3 py-1.5 rounded-xl">
              <Star className="w-3.5 h-3.5 fill-[#5B3FD4]" />
      ...
      <Card>
        <CardContent className="py-3 px-4">
          <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
            ...
            <span className="font-semibold text-[#5B3FD4]">
              {doneCount === 0
                ? 'Bora começar! 💪'
          ...
          <div className="w-full bg-gray-100 rounded-full h-2">
            <div
              ...
              style={{ width: `${Math.round((doneCount / tasks.length) * 100)}%`, backgroundColor: '#5B3FD4' }}
            />
          </div>
        </CardContent>
      </Card>
                  ...
                  >
                    <button
                      ...
                      className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all ${
                        task.done ? 'bg-[#52D68A] border-[#52D68A]' : 'border-gray-300 hover:border-[#5B3FD4]'
                      }`}
                    >
                      {task.done && (
                    ...
                    <span className={`flex items-center gap-0.5 text-xs font-semibold flex-shrink-0 ml-2 ${task.done ? 'text-[#52D68A]' : 'text-gray-400'}`}>
                      <Star className={`w-3 h-3 ${task.done ? 'fill-[#52D68A]' : ''}`} />
