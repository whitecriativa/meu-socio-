...
export function BomDiaCard({ name, message, revenue_yesterday, goal_percent }: BomDiaCardProps) {
  ...
  return (
    <Card className="bg-[#0F40CB] border-0 text-white overflow-hidden relative">
      {/* Decoração de fundo */}
      <div className="absolute top-0 right-0 w-40 h-40 rounded-full bg-white/5 -translate-y-10 translate-x-10" />
      <div className="absolute bottom-0 right-16 w-24 h-24 rounded-full bg-[#B6F273]/20 translate-y-8" />
      ...
      <CardContent className="p-5 relative">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="w-9 h-9 rounded-xl bg-[#B6F273] flex items-center justify-center flex-shrink-0">
